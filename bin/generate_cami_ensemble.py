...
def copy_DART_init_files(caminame,ensemble_start,ensemble_end,date,baserundir,outroot="b.e21.f09_g17.cam.i."):
    camiDir = "/glade/campaign/cesm/development/cross-wg/S2S/DART/atm_IC/{}-00000/".format(date)
    for imember in range(ensemble_start+1,ensemble_end+2):
        camiSrcFile="f.e21.FHIST_BGC.f09_025.CAM6assim.011.cam_{:04d}.i.{}-00000.nc".format(imember,date)
        infile  = os.path.join(camiDir,camiSrcFile)
        outfile = os.path.join(baserundir[:-2]+"{:02d}".format(imember-1),outroot+date+"-00000.nc")
        print(infile)
        print(outfile)
        if os.path.exists(outfile):
            os.unlink(outfile)
        print("Linking {} to {}".format(infile,outfile))
        rundir = os.path.dirname(outfile)
        if os.path.isdir(rundir):
            shutil.rmtree(rundir)
        os.makedirs(rundir)
