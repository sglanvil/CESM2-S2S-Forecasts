...
sys.path.append(_LIBDIR)
import random, threading, time, shutil, glob, re
...
def copy_DART_init_files(caminame,ensemble_start,ensemble_end,date,baserundir,outroot="b.e21.f09_g17.cam.i."):
    camiDir = "/glade/campaign/cesm/development/cross-wg/S2S/DART/lnd_IC/{}-00000/".format(date)
    for imember in range(ensemble_start+1,ensemble_end+2):
        rundir = baserundir[:-2]+"{:02d}".format(imember-1)
        print(rundir)
        ...
        for filename in file_list:
            print(filename)
            pattern = r'.*/f\.e21\.FHIST_BGC\.f09_025\.CAM6assim\.011\.(.*?)_{:04d}\.(.*?)\.{}-00000\.((?:nc)|(?:bin))'.format(imember, date)
...
def _main_func(description):
    ...
    restdir = os.path.join(os.getenv("SCRATCH"),workflow,"StageIC","rest","{}".format(date))
    baserundir = os.path.join(os.getenv("SCRATCH"),workflow+"_"+date+".00","run.00")
    caminame = os.path.join(restdir,"b.e21.f09_g17.cam.i.{date}-00000.nc".format(date=date))
...
#    create_cam_ic_perturbed(caminame,ensemble_start, ensemble_end, date,baserundir, outroot=outroot)
    copy_DART_init_files(caminame,ensemble_start,ensemble_end,date,baserundir,outroot=outroot)
